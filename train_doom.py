...
class DoomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Re-list files to get current state (more reliable than cached lengths)
        frame_files = sorted([f for f in os.listdir(ep_path) if f.endswith(".png")])
        json_files = sorted([f for f in os.listdir(ep_path) if f.startswith("frame_") and f.endswith(".json")])
        
        # Use the actual number of files available (handle mismatches safely)
        ep_len = min(len(frame_files), len(json_files))
        if ep_len == 0:
            # Return zero-padded batch if episode has no valid frames
            return torch.zeros(self.n_hist, 3, self.height, self.width), torch.zeros(self.n_hist, len(VALID_ACTIONS))
        
        # Calculate start index based on actual available frames
        start_idx = 0 if ep_len <= self.n_hist else random.randint(0, ep_len - self.n_hist)
        actual_n_hist = min(ep_len - start_idx, self.n_hist)  # Ensure we don't go out of bounds
...
def train_step_teacher_forced(vae, unet, scheduler, conditioner, noise_bucketer, caption_encoder, cfg, 
    ...
    alphas_cumprod = scheduler.alphas_cumprod.to(device=device, dtype=model_output.dtype)
    
    if cfg.prediction_type == "v_prediction":
        # V-prediction: train model to predict v, compute v_target from x0 and noise
        v_target = get_v_target(x0, noise, timesteps, alphas_cumprod)
        return F.mse_loss(model_output, v_target)
    elif cfg.prediction_type == "epsilon":
        # Epsilon prediction: train model to predict noise directly
        return F.mse_loss(model_output, noise)
    else:  # "sample" or default
        # Sample prediction: convert epsilon to x0 for loss computation
        # The pretrained SD model predicts epsilon, so we convert to x0 to supervise with clean image loss
        x0_pred = epsilon_to_x0(model_output, x_t, timesteps, alphas_cumprod)
            ...
            with torch.no_grad():
                ...
                for step in range(rollout_steps):
                    ...
                    for t in scheduler.timesteps:
                        # Model outputs prediction (epsilon, v, or sample depending on cfg.prediction_type)
                        model_pred = cfg_with_context_dropout(
                            unet, lat, ctx, t.expand(1), action_text_embeds, timestep_cond, cfg.cfg_weight
                        )
                        
                        lat = scheduler.step(model_pred, t, lat).prev_sample
...
'''
python train_doom.py --data_dir debug/rnd/20251031_223237 --batch_size 4 --steps 50000 --lr 3e-5 --prediction_type sample --gradient_checkpointing --rollout_interval 100  --use_adafactor --warmup_steps 1000 --hold_steps 20000 --full_ft
'''
